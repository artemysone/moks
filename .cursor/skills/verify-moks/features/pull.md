...
- `pull-cards` projects one card per application into the focused req's `candidates/` (the packet root in the single-req fixture); existing cards keep score and notes.
- `pull-status` shows the same counts through `moks status`.
- `pull-repeat` on a second pull is not a second seed and creates no duplicate cards.
...
- **First pull.** Sync the mock ATS. Run `scripts/moks.sh pull --json`. Exit code `0`. Stdout JSON has `ats` `mock`, `seeded` `true`, `upserted.jobs` `1`, `upserted.candidates` `5`, `upserted.applications` `5`, `cards.dir` `candidates` with five `cand_*` ids in `cards.created`, and `path` equal to `$COMPANY`.
- **Ledger files.** Confirm side effects. `$COMPANY/.moks/ledger.sqlite` and `$COMPANY/.moks/mock-ats.sqlite` exist. They are not under the git checkout. `$COMPANY/candidates/` holds the five projected `cand-*.md` cards next to `jordan-lee.md`.
...
- Pull seeds the **mock ATS fixture** (Jane Ortega, Priya Shah, …) and projects their cards; `jordan-lee` is a separate scoring fixture, not a mirror entity.
