...
export function createGreenhouseAdapter(db: SqliteDb, options: { fixturePath: string }): AtsAdapter {
  return {
    ...
    apply(change) {
      return applyGreenhouseChange(db, change);
    },
    transaction(fn) {
...
function readApplication(db: SqliteDb, id: string): Application | undefined {
  return db
    .prepare(
      "SELECT id, remote_id AS remoteId, job_id AS jobId, candidate_id AS candidateId, stage FROM greenhouse_applications WHERE id = ?",
    )
...
function readCandidate(db: SqliteDb, id: string): Candidate | undefined {
  return db
    .prepare("SELECT id, remote_id AS remoteId, name, email, headline FROM greenhouse_candidates WHERE id = ?")
    .get(id) as Candidate | undefined;
}

function readJob(db: SqliteDb, id: string): Job | undefined {
  return db
    .prepare("SELECT id, remote_id AS remoteId, title, team, location, status FROM greenhouse_jobs WHERE id = ?")
...
function casUpdateStage(db: SqliteDb, id: string, from: ApplicationStage, to: ApplicationStage): ApplyResult {
  const updated = db
    .prepare("UPDATE greenhouse_applications SET stage = ? WHERE id = ? AND stage = ?")
    .run(to, id, from);
...
export function applyGreenhouseChange(db: SqliteDb, change: ApplyChange): ApplyResult {
  const current =
  ...
  switch (change.mutation) {
    ...
    case "AddNote": {
      ...
      db.prepare(
        "INSERT INTO greenhouse_notes (id, entity_type, entity_ref, body, created_at) VALUES (?, ?, ?, ?, ?)",
    ...
    case "AddTag": {
      ...
      db.prepare(
        "INSERT OR IGNORE INTO greenhouse_tags (entity_type, entity_ref, tag, created_at) VALUES (?, ?, ?, ?)",
    ...
    case "SendOutreach": {
      ...
      db.prepare(
        "INSERT INTO greenhouse_outreach (id, entity_type, entity_ref, channel, body, created_at) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    case "ExtendOffer": {
      ...
      const id = crypto.randomUUID();
      db.prepare("INSERT INTO greenhouse_offers (id, entity_ref, terms, created_at) VALUES (?, ?, ?, ?)").run(
        id,
        change.entityRef,
        payload.terms,
        Date.now(),
