...
// etc.).
//
// The visual language is a curated subset of the OC-2 semantic tokens from the
// OpenCode lineage. Keep this file in sync with the TUI brand when it changes.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `moks is now connected to ${escapeHtml(provider)}.` : "moks is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `moks couldn't finish connecting to ${escapeHtml(provider)}.`
        : "moks couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from moks.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · moks</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("moks couldn't finish connecting to "+PROVIDER+"."):"moks couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from moks."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("moks is now connected to "+PROVIDER+"."):"moks is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="moks" role="img">
