...
- The server still lives in `packages/moks`.
...
Until enough shared service code lives outside `packages/moks`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/moks`

It should not import `packages/moks` services while `packages/moks`
...
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/moks` implementation details.
