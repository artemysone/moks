...
Delete `packages/moks/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/moks/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/moks/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/moks`
- TUI plugin loader tests in `packages/moks` if runtime plugin API wiring changed
