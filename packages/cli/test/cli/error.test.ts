...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/moks.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/moks.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/moks.jsonc", dir: ".moks", suggestion: "opencode" },
        expected:
          'Directory ".moks" in /tmp/moks.jsonc is not valid. Rename the directory to "opencode" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        expected: "Configuration is invalid at /tmp/moks.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
