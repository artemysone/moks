...
`,
  ...
  const pure = process.env.MOKS_PURE
  const meta = process.env.MOKS_PLUGIN_META_FILE
  process.env.MOKS_PURE = "1"
  process.env.MOKS_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.MOKS_PURE
    } else {
      process.env.MOKS_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.MOKS_PLUGIN_META_FILE
    } else {
      process.env.MOKS_PLUGIN_META_FILE = meta
    }
  }
})
