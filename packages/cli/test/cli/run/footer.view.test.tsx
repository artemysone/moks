...
import { MoksKeymapProvider, registerMoksKeymap } from "@moks/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerMoksKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerMoksKeymap(keymap, renderer, tuiConfig)
...
test("direct footer mode label keeps left padding without a status pill", async () => {
  ...
  try {
    ...
    const statusline = app
      .captureCharFrame()
      .split("\n")
      .find((line) => line.includes("RECRUIT") && line.includes("cmd"))

    expect(statusline).toBeDefined()
    expect(statusline?.startsWith(" RECRUIT ")).toBe(true)
  } finally {
    app.cleanup()
  }
})
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerMoksKeymap(keymap, renderer, tuiConfig)
