...
it.effect("score-candidate writes a score file path; commit-disposition cites it", () =>
  Effect.sync(() => {
    ...
    expect(PROMPT_INITIALIZE).toContain('never a yes/no "want a working name on the dossier"')
    expect(PROMPT_INITIALIZE).toContain("website URL")
    expect(PROMPT_INITIALIZE).toContain("public profile")
    expect(PROMPT_INITIALIZE).toContain("Do not invent a dossier from nothing")
