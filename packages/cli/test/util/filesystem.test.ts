...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "moks.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "moks.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "moks.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "moks.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["moks.json", "moks.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "moks.json"),
        path.join(project, "moks.jsonc"),
      ])
    })
  })
