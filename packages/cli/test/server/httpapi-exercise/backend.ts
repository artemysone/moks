    ...
    modules.HttpApiApp.routes.pipe(
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({ MOKS_SERVER_PASSWORD: password, MOKS_SERVER_USERNAME: username }),
        ),
      ),
    ),
...
function toAuthProbeRequest(scenario: ActiveScenario, credentials: "missing" | "valid", signal: AbortSignal) {
  ...
  if (spec.body !== undefined) {
    Object.assign(headers, { "content-type": "application/json" }, spec.headers)
  }
  if (credentials === "valid") headers.authorization = basic("opencode", "secret")
  return new Request(new URL(spec.path, "http://localhost"), {
    method: scenario.method,
    headers,
    body: spec.body === undefined ? undefined : JSON.stringify(spec.body),
