...
it.instance("recruit agent has correct default properties", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("bash", "moks push --commit-id abc", recruit!.permission).action).toBe("deny")
    expect(Permission.evaluate("bash", "moks review abc --approve --by you", recruit!.permission).action).toBe("deny")
    ...
    expect(evalPerm(recruit, "diff")).toBe("allow")
    expect(evalPerm(recruit, "push")).toBe("deny")
    expect(evalPerm(recruit, "review")).toBe("deny")
    expect(Permission.evaluate("edit", ".moks/ledger.sqlite", recruit!.permission).action).toBe("deny")
    expect(Permission.evaluate("edit", ".moks/vault.key", recruit!.permission).action).toBe("deny")
...
it.instance("recruit agent allows edit under product hiring fixtures", () =>
  Effect.gen(function* () {
    ...
    const monorepoRel = "packages/cli/src/product/fixtures/hiring/HIRING.md"
