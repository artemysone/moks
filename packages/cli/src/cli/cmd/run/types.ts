...
import type { MoksClient, PermissionRequest, QuestionRequest, ToolPart } from "@moks/sdk/v2"
...
type PromptModel = Parameters<MoksClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<MoksClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<MoksClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<MoksClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<MoksClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<MoksClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<MoksClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<MoksClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<MoksClient["question"]["reject"]>[0]
