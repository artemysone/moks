...
// CLI entry point for `moks run` and `moks --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`moks --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`moks --mini --attach`): connects to a running
//      moks server and runs interactive mode against it.
//
...
export const RunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: MoksClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: MoksClient, sessionID: string) {
      ...
      async function current(sdk: MoksClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: MoksClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: MoksClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: MoksClient) {
        ...
        async function loop(client: MoksClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
