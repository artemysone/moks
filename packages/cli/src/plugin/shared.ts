...
export async function checkPluginCompatibility(target: string, hostVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(hostVersion) || semver.major(hostVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = typeof engines.moks === "string" ? engines.moks : engines.opencode
  if (typeof range !== "string") return
  if (!semver.satisfies(hostVersion, range)) {
    throw new Error(`Plugin requires moks ${range} but running ${hostVersion}`)
  }
}
