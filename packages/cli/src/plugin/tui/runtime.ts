...
import { errorData, errorMessage } from "@moks/tui/util/error"
...
import { hasTheme, upsertTheme } from "@moks/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@moks/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@moks/core/v1/config/plugin"
import { createCommandShim } from "@moks/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@moks/tui/plugin/runtime"
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.MOKS_PURE ? [] : pluginOrigins
    if (Flag.MOKS_PURE && pluginOrigins.length) {
    }
