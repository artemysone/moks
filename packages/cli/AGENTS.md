...
- **Schema**: Drizzle schema lives in `packages/engine/core/src/**/*.sql.ts`.
- **Migrations**: database migrations live in `packages/engine/core` and are applied by core.

## Development server

- Running `bun dev` from `packages/cli` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
