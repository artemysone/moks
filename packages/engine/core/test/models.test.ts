...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.MOKS_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.MOKS_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.MOKS_MODELS_PATH = undefined
  Flag.MOKS_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.MOKS_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.MOKS_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.MOKS_DISABLE_MODELS_FETCH = false
        }),
