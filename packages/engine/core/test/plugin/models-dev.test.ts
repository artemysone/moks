...
import { Integration } from "@moks/core/integration"
...
import { ModelsDevPlugin } from "@moks/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.MOKS_MODELS_PATH,
          disabled: Flag.MOKS_DISABLE_MODELS_FETCH,
        }
        Flag.MOKS_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.MOKS_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.MOKS_MODELS_PATH = previous.path
          Flag.MOKS_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
