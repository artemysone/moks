...
import { PermissionTable } from "@moks/core/permission/sql"
...
describe("PermissionV2", () => {
  ...
  it.effect("uses recruit permissions when the Session agent is omitted", () =>
    Effect.gen(function* () {
      yield* setup()
      const { db } = yield* Database.Service
      yield* db
      ...
      const agents = yield* AgentV2.Service
      yield* agents.transform((editor) =>
        editor.update(AgentV2.ID.make("recruit"), (agent) => {
  ...
  it.effect("denies omitted-agent permissions when no primary default agent exists", () =>
    Effect.gen(function* () {
      ...
      yield* agents.transform((editor) => {
        ...
        editor.remove(AgentV2.ID.make("recruit"))
      })
