...
const snapshot = path.join(pkg, "schema.json")
const tsDir = path.join(pkg, "src/database/migration")
const registry = path.join(pkg, "src/database/migration.gen.ts")
const schema = path.join(pkg, "src/database/schema.gen.ts")
...
async function check() {
  ...
  try {
    ...
    if ((await generatedMigrations(incremental)).length > 0) {
      throw new Error(
        "Core schema has ungenerated database migrations. Run `bun script/migration.ts` from packages/engine/core.",
      )
    }
    ...
    if ((await Bun.file(schema).text()) !== (await formatTypescript(renderSchema(await generatedSql(full))))) {
      throw new Error("Current database schema is stale. Run `bun script/migration.ts` from packages/engine/core.")
    }
    ...
    if ((await Bun.file(registry).text()) !== (await formatTypescript(renderRegistry(migrations)))) {
      throw new Error("Database migration registry is stale. Run `bun script/migration.ts` from packages/engine/core.")
    }
  } finally {
    await fs.rm(temporary, { recursive: true, force: true })
  }
...
async function drizzle(temporary: string, output: string, name?: string) {
  ...
  await Bun.write(
    config,
    `import config from ${JSON.stringify(pathToFileURL(path.join(pkg, "drizzle.config.ts")).href)}
