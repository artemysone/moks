...
export const MOKS_BASE_MODE = "base"
...
export function createMoksModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(MOKS_MODE_KEY, MOKS_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(MOKS_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(MOKS_MODE_KEY, stack.at(-1)?.mode ?? MOKS_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? MOKS_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(MOKS_MODE_KEY, undefined)
...
export function registerMoksKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createMoksModeStack(keymap)
