...
import type { useMoksKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useMoksKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getMoksModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getMoksModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
